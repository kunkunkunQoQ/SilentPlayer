...
## 2026-09-20（第三批收尾：T4/T5/T6/T8/T9）

### Added

- **T4 长文件名悬停显示完整名**：文件名标签是 `SS_ENDELLIPSIS` 截断的，此前**全项目没有任何 tooltip**，
  截断后无从查看。现在挂了一个 tooltip 控件（`TTF_IDISHWND | TTF_SUBCLASS`，
  不用手工维护矩形；切换文件时用 `TTM_UPDATETIPTEXT` 更新文本）。

### Fixed

- **T6 高对比度适配**：副标题/时间/百分比的灰色原本是硬编码 `RGB(110,110,110)`，
  高对比度主题下可能不可读 → 改用 `GetSysColor(COLOR_GRAYTEXT)`（常规主题下观感基本不变）。
- **T5 退出时清理按应用路由**：`ClearProcessOutputDevice` 原先只在启动/切换时调用，
  开着「输出到麦克风」退出会在系统里留下持久化设置 → 现在退出时也清一次，不留痕迹。
- **T8 空闲时副标题不再带模式后缀**：之前空闲态会显示 `未打开文件 · 输出到麦克风`，读起来别扭
  → 只有真正在播放某个文件时才加后缀。
- **T9 `--volume=abc` 不再被当成 0%**：原先 `_wtoi` 解析失败返回 0 且不校验，等于静音
  → 改用 `wcstol` + 完整校验，非法值直接忽略（`--volume=55` 仍正常生效）。

### Verified

- **T4**：`--tooltipcheck` 实测 `tooltip window = exists` ✓
  （注意：tooltip 是 WS_POPUP，主窗口只是**属主**不是父窗口，探针要按"同进程 + 类名"找，
  不能用 `FindWindowEx(hwnd,...)` 搜子窗口——第一版检查就写错了）
- **T8**：空闲态开麦前后副标题都是 `未打开文件`（无后缀）✓
- **T9**：起始 80% →（`--volume=abc`）→ 仍 **80%**（被忽略）→（`--volume=55`）→ **55%** ✓
- 构建：**零错误零警告** ✓

## 2026-09-20（第三批 T1/T2/T3：打开文件入口 + 滚轮/点击手感一致）

### Added

- **T1 托盘菜单「打开文件…」**（菜单第一项，**始终可用**：不论是否在播放、窗口是否隐藏）：
  用系统 `IFileOpenDialog` 选音频文件并播放。**此前全项目没有任何文件选择对话框**，
  打开文件只能靠命令行 / 拖到窗口 / 双击文件关联——而窗口平时是隐藏的，等于没有可发现的入口。
  过滤：`*.mp3;*.wav;*.mp4;*.m4a;*.aac;*.flac;*.wma` + 所有文件。

### Changed

- **T2 滚轮统一在整窗调音量**：`WM_MOUSEWHEEL` 改到**消息循环**里统一处理（与键盘快捷键同一处），
  因此光标落在**任何**控件上都一样（进度条 / 音量条 / 按钮）。
  顺带修掉一个真实手感问题：光标在**进度条**上滚轮时，trackbar 原本会按 `TBM_SETLINESIZE=20`
  把滑块挪 2%（≈2.4 秒）——**用户本意是调音量，却改掉了播放位置**。
- **T3 音量条也支持"点哪跳哪"**：与进度条一致（N2 只做了进度条，同一窗口两种手感）。
  实现上把进度条的跳转逻辑抽成共用的 `JumpTrackToX()`，两条滑块各用一份子类化过程
  （不借用 `GWLP_USERDATA`，避免与控件自身用途冲突），跳转后统一按 `TB_ENDTRACK` 通知父窗口
  —— 走与"拖动结束"完全相同的路径（进度条→seek，音量条→SetVolume）。

### Verified

- **T1**：菜单实测 12 项，`[0] id=7`＝「打开文件…」；触发后**系统文件对话框确实弹出**
  （`file dialog appeared = yes`），关闭后进程存活 ✓
- **T2**：把滚轮投给**进度条**（模拟光标在子控件上）：音量 80% →（3 格）→ **95%**（精确 3×5%），
  进度 6→46/1000 只是随时间自然前进、**没有台阶跳** ✓
- **T3**：点音量条 30% → **0.28**；点 70% → **0.71** ✓
- 回归：格式矩阵（t.mp3 / t.flac / t.mp4 / t.wma / mp4content.mp3）全部 PLAYING，
  broken → BADFILE ✓；干净重建 **零错误零警告**；EXE 315,904 字节 ✓

### 顺带查清了那个"间歇性整批 BADFILE"（重要）

前几轮出现过的"整批格式全部 BADFILE、重跑即恢复"现象，这次被新加的**自诊断**抓到了错误码：

```
错误码：0x80070002 （找不到该文件）
```

即 **`ERROR_FILE_NOT_FOUND`** —— 不是解码/代码问题，而是**当时文件确实取不到**。
已排除：样本存在且完好、E: 是本地固定 NTFS 盘（非网络/可移动盘）、单次运行与重跑都正常。
同时发现测试脚本本身有个坑：bash 里 `S="E:////kunkun////..."` 再拼 `"$S\\$F"`，
**MSYS 会把 `\\` 变成 `//`**（`echo` 实测输出 `E://kunkun//...`），传给应用的是正斜杠路径。
建议脚本改用**单引号包裹的 Windows 路径**，既不被转换也更接近真实用户场景。
用这种写法重跑矩阵**全部正常**。

## 2026-09-20（第二批低成本增强：N 档 7 项，全部与"日常体验"直接相关）

用户要求跳过 B/C/D，改做**其他便宜但体验提升明显**的改动。逐条核对代码后选了 7 项。

### Added

| 项 | 内容 | 为什么体验提升明显 |
| --- | --- | --- |
| N1 | **托盘左键单击切换显示/隐藏**（原来只"显示"，隐藏必须去点关闭按钮） | 托盘常驻程序的标准操作习惯 |
| N2 | **进度条"点哪跳哪"**：点轨道任意位置直接跳到该位置 | 原生 trackbar 点轨道只会挪一页；拖一个细滑块很难精确 |
| ~~N3~~ | ~~**系统媒体键**：键盘上的「播放/暂停」「停止」直接控制~~ | **已移除**（见下） |
| N4 | **错误提示讲人话**：错误码后附原因，如 `0xC00D36C4（格式不受支持，或文件已损坏）` | 普通用户看不懂十六进制 |
| N5 | **Esc 隐藏窗口** | 键盘用户的常规预期 |
| N6 | **托盘菜单显示快捷键提示**（`播放	空格 / 媒体键`） | 让 A4/N3 的快捷键可被发现 |
| N7 | 显示窗口时 `BringWindowToTop` 兜底（`SetForegroundWindow` 可能被系统拒绝） | 避免窗口"弹在别的窗口后面" |

### 实现要点

- **进度条子类化**（`SetWindowLongPtrW(GWLP_WNDPROC)`，不用 comctl32 v6 的 `SetWindowSubclass`）：
  先让原生处理跑（它会开始拖动跟踪），再把位置改到点击处并**按 `TB_ENDTRACK` 通知父窗口**
  —— 也就是走与"拖动结束"完全相同的 seek 路径，不新增第二条逻辑。这样"点哪跳哪"和
  "按住继续拖动"都可用。
- 托盘左键单击切换 + 双击确保显示（双击会先触发一次 UP，所以双击时保证最终是显示状态）。

### Verified

- **N1**：单击托盘 → `visible=0` → 再单击 → `visible=1` → 再单击 → `0` ✓
- **N2**：点 50% → **01:02**（总长 02:02，正好一半）；点 25% → 00:29；点 75% → 01:34 ✓
- **N4**：损坏文件的弹窗实测为 `错误码：0xC00D36C4 （格式不受支持，或文件已损坏）` ✓
- **N5**：按 Esc → `visible=0` ✓
- **N6**：菜单项文本实测含 `\t空格` ✓
- **回归**：9 种格式 + 错误扩展名 + 损坏文件全部正常；托盘「停止 / 销毁 / 输出到麦克风 / 退出」
  全部命中（退出码 0）；「输出到麦克风」链路正常（麦克风 peak 0.8061）✓
- Debug / Release **干净重建**，**零错误、零警告**；EXE 314,880 字节 ✓
- **崩溃残留修复实测**：开启「输出到麦克风」（麦克风 peak 0.7999）→ 强制结束 → 重启 →
  渲染端点 = `Sonar - Gaming`（系统默认）、麦克风 **0.0000**、状态 `PLAYING` ✓

### Fixed（本轮抓到的真实缺陷）

**崩溃残留会让下次启动听不到声音**：如果上次是「输出到麦克风」状态被**强制结束**
（崩溃 / 任务管理器结束），按应用路由设置会残留在系统里，下次启动播放器仍被路由到虚拟麦克风
→ 用户完全听不到声音。

排查过程（值得记住）：
1. 最初在"建窗口之前"清一次 —— 无效（清除是**异步**生效的，与首次加载挨太近来不及）。
2. 改成"清完立刻加载" —— 无效。
3. 改成"加载后立刻清 + 重载" —— 会与首次加载抢会话，**偶发重载失败并报 BADFILE**（我自己引入的缺陷）。
4. 改成"清 + 轮询到读取接口报告已清除再加载" —— 无效：读取接口报告已清除，但**实际路由仍指向麦克风**
   （说明 Sonar 这类软件是在**本进程启动时**重新应用它自己的设置，晚于我们的清除）。
5. **最终方案**：启动时先清一次并**立刻加载**（让用户尽快听到声音），
   再用一个 900ms 的一次性定时器执行"**清 + 重载**"—— 也就是与托盘开关关闭**完全相同**的路径，
   只是延迟到首次加载稳定之后。实测有效且稳定。

### Removed（N3 系统媒体键 —— 用户要求移除）

最初用 `RegisterHotKey` 注册了 `VK_MEDIA_PLAY_PAUSE` / `VK_MEDIA_STOP`，让键盘媒体键直接控制播放。
**但 `RegisterHotKey` 是全局独占**：一旦注册，其它播放器（以及系统本身的媒体键处理）就收不到这两个键了
——对"只是偶尔放个音频"的静默播放器来说，抢走全局媒体键的代价明显大于收益，因此**整项移除**。

移除内容：`App::RegisterMediaKeys/UnregisterMediaKeys/HandleHotkey`、`PlayerWindow` 的 `WM_HOTKEY` 分支、
以及菜单提示里的"媒体键"字样。**空格 / ←/→ / Esc / 滚轮这些只在本窗口内生效的快捷键全部保留。**

实测确认已释放：外部进程现在可以成功注册 `VK_MEDIA_PLAY_PAUSE`（`ok=1, err=0`）。

### 测试方法修正（重要）

批量脚本里"`taskkill` 后固定 sleep"**不可靠**：新实例可能把文件转发给**正在退出的旧实例**，
旧实例在关闭过程中加载失败 → 探针读到 `BADFILE` 的**假失败**（本轮又误判一次）。
正确做法是**等进程真正消失**再启动下一个：
...
## 2026-09-20（执行 ROADMAP 的 A 档：7 项低成本增强）

按 [ROADMAP.md](ROADMAP.md) 的 A 档实施，全部不改架构、不加依赖。

### Added

| 项 | 内容 |
| --- | --- |
| A1 | **托盘悬停显示当前播放**：`SilentPlayer — 文件名（正在播放）`。用的是此前一直没被调用的 `TrayIcon::SetTooltip()` |
| A2 | **副标题显示输出模式**：开启「输出到麦克风」时显示 `正在播放 · 输出到麦克风`，避免用户以为"没声音了" |
| A3 | **鼠标滚轮调音量**：窗口上滚轮 ±5%（走与滑块相同的设置路径） |
| A4 | **键盘快捷键**：空格 = 播放/暂停，←/→ = 快退/快进 5 秒 |
| A5 | **托盘菜单顶部显示当前文件名**（灰色禁用项） |
| A6 | **命令行控制命令**：`--toggle` / `--stop` / `--show` / `--exit` / `--volume=<0-100>`，由已运行实例执行（可给脚本 / Stream Deck 用） |
| A7 | **关闭窗口后一次性气泡提示**：`仍在后台播放，左键托盘图标可再次打开`（`NIIF_NOSOUND` 静音气泡，仅本次运行提示一次） |

### Changed

- `App` 新增统一的 `SetStatusText()`：状态副标题与托盘提示**只在这一处拼装**
  （原先 7 处各自调用 `m_window.SetStatus`，容易漏改）。
- 键盘快捷键在**消息循环**里统一拦截，因此焦点在窗口还是子控件上都生效
  （本程序没有文本输入，不会与输入冲突）。
- `SingleInstance` 抽出 `SendToExisting(magic, payload)`：文件路径与**控制命令**共用同一转发通道
  （`"SPLR"` / `"SPLC"` 两个魔数），原有"传文件路径"的语义未变。

### 探针改进（测试手段）

- `player_state_probe` 的 `--trayitem` 从"键盘下移计数"改为**按菜单项屏幕坐标直接点击**：
  键盘计数会受"新增的禁用项/分隔符是否可导航"影响而错位，坐标点击完全确定。
  参数语义随之明确为 **`--trayitem=<采样点>:<菜单位置>`**（位置 0 起，见 `--traydump`）。
- 新增 `--wheel=<刻度>`、`--key=space|left|right`（模拟滚轮与按键）。
- `--dump` 现在会把控件清单以 UTF-8 落盘到 `build/_controls.txt`（控制台代码页显示不了中文）。
- 新增 `test/tray_probe.cpp`：`rect` 打印托盘图标矩形，`hover <bmp>` 悬停后截取该区域。

### Verified

- **A2**：开启后副标题实测 `正在播放 · 输出到麦克风`；菜单项显示 `[CHECKED]`；关闭后回到 `正在播放` ✓
- **A3**：80% →（上滚 2 格）→ **90%** →（下滚 4 格）→ **70%** ✓
- **A4**：空格 → `PAUSED`；再空格 → `PLAYING`；→ 键 → 进度 00:12 → 00:20 ✓
- **A5**：菜单顶部实测出现文件名项 `[0] id=0 text='t.flac'` ✓
- **A6**：`--toggle` → PAUSED、`--volume=35` → 音量变化、`--stop` → STOPPED、
  `--exit` → 退出码 **0**；第二实例转发后自行退出（进程数恒为 1）✓
- **回归**：9 种格式 + 错误扩展名 + 损坏文件全部正常；托盘「停止 / 销毁 / 输出到麦克风 / 退出」
  全部命中（退出码 0）；**「输出到麦克风」音频链路未受影响**（麦克风 0.0000 → 0.8079 → 0.0000）✓
- Debug / Release **干净重建**，**零错误、零警告**；EXE 312,320 字节 ✓

### 未自动验证（需人工看一眼）

- **A1 的 tooltip 与 A7 的气泡**属于系统绘制的提示，自动化截图没能稳定捕捉到
  （Windows 11 托盘图标位置动态、提示出现时机难抓）。代码路径已接好，**请手动悬停托盘图标 /
  点一次关闭按钮确认**。
- A7 与项目原则「默认静默、无 Toast」有张力：已用**静音气泡 + 仅本次运行一次**降到最低干扰；
  若不希望有任何气泡，说一声即可去掉。
